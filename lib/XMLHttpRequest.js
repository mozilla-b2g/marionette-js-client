...
exports.XMLHttpRequest = function() {
  ...
  this.send = function(data) {
    ...
    if(!settings.hasOwnProperty("async") || settings.async) {
      ...
      var syncDataFile = syncFile + '-data';

      fs.writeFileSync(syncFile, "", "utf8");
      fs.writeFileSync(syncDataFile, '', 'utf8');

      if (data) {
        fs.writeFileSync(syncDataFile, JSON.stringify(data), 'utf8');
      }
      ...
      var execString = "var http = require('http'), https = require('https'), fs = require('fs');"
        ...
        + "fs.writeFileSync('" + syncFile + "', 'NODE-XMLHTTPREQUEST-ERROR:' + JSON.stringify(error), 'utf8');"
        + "});"
        + (data ? "req.write(JSON.parse(fs.readFileSync(__dirname + '/' + '" + syncDataFile + "').toString()));": "")
        + "req.end();";

      debug('sync string', execString);
      ...
      //fs.unlinkSync(syncDataFile);
      console.log('WTF MAN I GOT THIS:', self.responseText);
