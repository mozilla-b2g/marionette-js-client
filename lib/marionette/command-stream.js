(function(module, ns) {
  ...
  proto._checkBuffer = function _checkBuffer() {
    ...
    if (!this.inCommand) {
      ...
      if (lengthIndex !== -1) {
        ...
        var prefixLength = String(this.commandLength).length + 1;
        this.bufferSize -= prefixLength;
  ...
  proto._readBuffer = function _readBuffer() {
    ...
    if (this._checkBuffer()) {
      if (this.bufferSize >= this.commandLength) {
        // note slice here is slightly wrong but it does not matter because
        // marionette responses are in serial order so we won't receive more
        // buffer data until the current buffer is completely filled.
        ...
        this.inCommand = false;
        this.bufferSize -= this.commandLength;
