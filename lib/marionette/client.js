(function(module, ns) {
  ...
  Client.prototype = {
    ...
    currentFrame: function(callback) {
      ...
      var currentLocation = this.executeScript(function() {
        return window.location;
      });

      // Switch to top-level iframe.
      this.switchToFrame();

      // Look through children for iframe with our origin.
      var childFrames = this.findElements('iframe');
      var result;
      childFrames.some(function(childFrame) {
        var location = childFrame.scriptWith(function(iframe) {
          return iframe.contentWindow.location;
        });

        if (location.origin === currentLocation.origin) {
          result = childFrame;
          return true;
        }
      });
      ...
      this.switchToFrame(result);
      this.defaultCallback(null, result);
    },
