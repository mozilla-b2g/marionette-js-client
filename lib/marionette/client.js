(function(module, ns) {
  ...
  Client.prototype = {
    ...
    /**
     * Same interface as waitFor but polls test function
     * in gecko instead of node. Your test will be stringified!
     */
    waitForRemote: function(test, options, callback) {
      if (typeof options === 'function') {
        callback = options;
        options = null;
      }
      ...
      var sync = this.isSync,
          options = options || {},
          callback = callback || this.defaultCallback,
          timeout = Date.now() + (options.timeout || DEFAULT_WAIT_FOR_TIMEOUT),
          interval = options.interval || DEFAULT_WAIT_FOR_INTERVAL;

      if (typeof test === 'function') {
        test = test.toString();
      }

      var poll = sync ? this.waitForRemoteSync : this.waitForRemoteAsync;
      return poll.call(this, test, callback, interval, timeout);
    },

    waitForRemoteSync: function(test, callback, interval, timeout) {
      return this.executeAsyncScript(function poll(fn, interval, timeout) {
        if (timeout <= 0) {
          return marionetteScriptFinished(
            new Error('waitForRemote timeout exceeded');
          );
        }
        ...
        if (error || pass) {
          return marionetteScriptFinished(error);
        }

        return setTimeout(function() {
          poll(fn, interval, timeout - interval);
        }, interval);
      }, [
        test,
        interval,
    ...
    waitForRemoteAsync: function(test, callback, interval, timeout) {
      return this.executeAsyncScript(function poll(fn, interval, timeout) {
        if (timeout <= 0) {
          return marionetteScriptFinished(
            new Error('waitForRemote timeout exceeded');
